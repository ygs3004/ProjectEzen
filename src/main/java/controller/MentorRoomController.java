...
//            request.setAttribute("message", "스터디는 한개만 개설할 수 있습니다.");
    ...
    public String createRoom(HttpSession session, MentorRoom mentorRoom, Model model){
        this.user_id = (String)session.getAttribute("user_id");
        mentorRoom.setNum(0);
        mentorRoom.setUser_id(user_id);
        mentorRoomService.createRoom(mentorRoom);
        int num = mentorRoomService.getRoomNoByID(this.user_id); //id로 만들어진 roomNum 조회
        mentorRoomService.usersAddRoomNo(num, user_id); //users MentorRoomNo에 roomNo에 추가
    ...
    public String modifyRoom(HttpSession session, Model model){
        this.user_id = (String)session.getAttribute("user_id");
        session.setAttribute("user_id", "user_id");
        model.addAttribute("mentorRoom",mentorRoomService.getRoomInfoByID(user_id));
